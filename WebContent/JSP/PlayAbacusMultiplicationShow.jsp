	...
	<form action="abacusMultiplicationServlet" method="post">
		...
		<% }
			if(numberList!=null && multiplicationList!=null && !numberList.isEmpty())
			{
				...
				<thead>
					<tr>
						<td style="background-color: blue; text-align: left; padding-left: 44%;" colspan="<%=(5*3)%>">
							<h3>ABACUS</h3>
						</td>
					</tr>
				</thead>
					...
					while(iterator.hasNext())  // iterate through all the data until the last record
					{
						if(columnCount==0 || (columnCount%5 == 0)) {
							...
							<%} %>
								...
								<td class="displayNumberStyle4">
									<input type="number"  class="numberInputTagStyle" id="product<%=rowCount %>" onblur="checkValue(this,'hiddenProduct<%=rowCount %>')">
									<input disabled type="text" style="display: none;" class="hiddenClass numberInputTagStyle" id="hiddenProduct<%=rowCount %>" value="<%=iterator1.next()%>">
								</td>
							<%
						} else {
							%>
							...
							<td class="displayNumberStyle4">
								<input type="number"  class="numberInputTagStyle" id="product<%=rowColumnCount+rowCount %>" onblur="checkValue(this,'hiddenProduct<%=rowColumnCount+rowCount %>')">
								<input disabled type="text" style="display: none;" class="hiddenClass numberInputTagStyle" id="hiddenProduct<%=rowColumnCount+rowCount %>" value="<%=iterator1.next()%>">
							</td>
						<%
						}
						columnCount++;
