 ...
 */
@WebServlet("/abacusMultiplicationServlet")
public class AbacusMultiplicationServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AbacusMultiplicationServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("Play Abacus".equalsIgnoreCase(submitButtonValue)
				|| "Play More".equals(showMoreData)) {
			request.getRequestDispatcher("JSP/PlayAbacusMultiplicationShow.jsp").forward(request, response);
		} else {
			request.getRequestDispatcher("JSP/AbacusMultiplicationShow.jsp").forward(request, response);
		}
	}
	...
	private static void positiveFixedDigitMultiplicationNumber(HttpServletRequest request
			...
			request.getSession().setAttribute("MultiplicationList", multiplicationList);
	}
